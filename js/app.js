// ============================================================
// SUOMI ÖĞRENİYORUM - Ana Uygulama Motoru (v2 - Eğitim Kategorisi)
// ============================================================
...
class FinnishApp {
  ...
  getDataSource(contentType, level) {
    if (contentType === 'words')     return (typeof VOCABULARY !== 'undefined'       ? VOCABULARY[level]       : null) || [];
    if (contentType === 'sentences') return (typeof SENTENCES !== 'undefined'        ? SENTENCES[level]        : null) || [];
    if (contentType === 'school')    return (typeof SCHOOL_SENTENCES !== 'undefined' ? SCHOOL_SENTENCES[level] : null) || [];
    if (contentType === 'math')      return (typeof MATH_VOCABULARY !== 'undefined'  ? MATH_VOCABULARY[level]  : null) || [];
    return [];
  }

  startActivity(level, mode, contentType) {
    const data = this.getDataSource(contentType, level);
    if (!data || data.length === 0) { alert('Bu seviyede henüz içerik yok!'); return; }
    const shuffled = [...data].sort(() => Math.random() - 0.5).slice(0, mode === 'flashcard' ? 20 : 10);
    this.state = {
      ...this.state,
      screen: 'activity', level, mode, contentType,
      currentItems: shuffled, currentIndex: 0, score: 0,
      streak: 0, lives: 3, sessionCorrect: 0, sessionTotal: 0,
  ...
  generateQuestion(item, mode, contentType, allData) {
    if (!item) return null;
    if (mode === 'flashcard') return { type: 'flashcard', item, revealed: false };
    if (mode === 'quiz') {
  ...
  handleFlashcard(rating) {
    ...
    this.addXP(xpMap[rating] || 5);
    if (rating !== 'hard') { this.state.sessionCorrect++; this.state.streak++; }
  ...
  handleQuizAnswer(selectedIndex) {
    ...
    q.selected = selectedIndex; q.answered = true; q.correct = correct;
    if (correct) { this.state.sessionCorrect++; this.state.streak++; this.state.score += 10; this.addXP(10); }
    else { this.state.streak = 0; this.state.lives = Math.max(0, this.state.lives - 1); }
    this.state.sessionTotal++;
    this.render();
  ...
  handleMatchSelect(type, id) {
    ...
    if (type === 'fi') q.selectedFi = q.selectedFi === id ? null : id;
    else               q.selectedTr = q.selectedTr === id ? null : id;
    if (q.selectedFi !== null && q.selectedTr !== null) {
      ...
      if (fiItem && trItem && fiItem.id === trItem.id) {
        q.matched.add(fiItem.id); q.selectedFi = null; q.selectedTr = null;
        this.state.sessionCorrect++; this.state.streak++; this.addXP(15);
        if (q.matched.size === q.items.length) setTimeout(() => this.nextItem(), 800);
      } else {
        this.state.streak = 0;
        setTimeout(() => { q.selectedFi = null; q.selectedTr = null; this.render(); }, 600);
      }
    }
    this.state.sessionTotal++;
    this.render();
  ...
  handleListeningAnswer(selectedIndex) {
    ...
    const correct = q.options[selectedIndex].id === q.item.id;
    q.selected = selectedIndex; q.answered = true; q.correct = correct;
    if (correct) { this.state.sessionCorrect++; this.state.streak++; this.addXP(12); }
    else { this.state.streak = 0; this.state.lives = Math.max(0, this.state.lives - 1); }
    this.state.sessionTotal++;
    this.render();
    setTimeout(() => this.nextItem(), 1500);
  ...
  nextItem() {
    ...
    if (next >= this.state.currentItems.length || this.state.lives === 0) { this.endSession(); return; }
    this.state.currentIndex = next;
    const allData = this.getDataSource(this.state.contentType, this.state.level);
    this.state.currentQuestion = this.generateQuestion(this.state.currentItems[next], this.state.mode, this.state.contentType, allData);
    this.render();
  }

  endSession() {
    const accuracy = this.state.sessionTotal > 0 ? Math.round((this.state.sessionCorrect / this.state.sessionTotal) * 100) : 0;
    this.progress.lastStudyDate = new Date().toDateString();
    this.saveProgress();
    this.navigate('results', { accuracy, xpEarned: this.state.score || this.state.sessionCorrect * 10 });
  }

  addXP(amount) {
    this.progress.totalXP = (this.progress.totalXP || 0) + amount;
    this.state.score = (this.state.score || 0) + amount;
    this.progress.level = Math.floor(this.progress.totalXP / 500) + 1;
    this.saveProgress();
  }

  speak(text) { this.audio.speak(text, 'fi-FI'); }
  ...
  renderHome() {
    ...
    <div class="screen home-screen">
      <div class="home-header">
        <div class="app-logo"><span class="logo-fi">FI</span><span class="logo-nce">nce</span></div>
        <button class="icon-btn" data-action="profile">👤</button>
      </div>
      <div class="hero-section">
        <div class="hero-text"><h1>Merhaba! 👋</h1><p>Fince öğrenmeye devam et</p></div>
        <div class="stats-row">
      ...
      <div class="level-cards">
        ${['A1','A2','B1','B2'].map(lvl => `
          <button class="level-card" data-action="selectLevel" data-level="${lvl}" data-type="words">
            ...
            <div class="level-info"><div class="level-name">${this.getLevelName(lvl)}</div><div class="level-count">${(VOCABULARY[lvl]||[]).length} kelime</div></div>
      ...
      <div class="level-cards">
        ${['A1','A2','B1','B2'].map(lvl => `
          <button class="level-card" data-action="selectLevel" data-level="${lvl}" data-type="sentences">
            <div class="level-badge level-${lvl.toLowerCase()}">${lvl}</div>
            <div class="level-info"><div class="level-name">${this.getLevelName(lvl)}</div><div class="level-count">${(SENTENCES[lvl]||[]).length} cümle</div></div>
            <div class="level-arrow">›</div>
          </button>`).join('')}
      </div>

      <div class="section-title">🏫 Okul Diyalogları</div>
      <div class="level-cards">
        ${['A1','A2','B1'].map(lvl => `
          <button class="level-card" data-action="selectLevel" data-level="${lvl}" data-type="school">
            <div class="level-badge level-${lvl.toLowerCase()}">${lvl}</div>
            <div class="level-info"><div class="level-name">${this.getLevelName(lvl)}</div><div class="level-count">${(typeof SCHOOL_SENTENCES!=='undefined'?SCHOOL_SENTENCES[lvl]:[]).length} diyalog</div></div>
            <div class="level-arrow">›</div>
          </button>`).join('')}
      </div>

      <div class="section-title">🔢 Matematik Terimleri</div>
      <div class="level-cards">
        ${['A1','A2','B1','B2'].map(lvl => `
          <button class="level-card" data-action="selectLevel" data-level="${lvl}" data-type="math">
            <div class="level-badge level-${lvl.toLowerCase()}">${lvl}</div>
            <div class="level-info"><div class="level-name">${this.getLevelName(lvl)}</div><div class="level-count">${(typeof MATH_VOCABULARY!=='undefined'?MATH_VOCABULARY[lvl]:[]).length} terim</div></div>
            <div class="level-arrow">›</div>
          </button>`).join('')}
      </div>
  ...
  renderModeSelect() {
    ...
    const labels = { words:'Kelime', sentences:'Cümle', school:'Okul Diyaloğu', math:'Matematik Terimi' };
    const modes = [
    ...
    <div class="screen">
      <div class="screen-header">
        ...
        <h2>${level} - ${labels[contentType]||'İçerik'} Aktiviteleri</h2>
      </div>
      <div class="mode-grid">
        ${modes.map(m => `
    ...
    <div class="flashcard-container">
      <div class="flashcard ${revealed?'revealed':''}" data-action="revealCard">
        ...
        <div class="card-back-content">
          ...
          ${item.tag ? `<div class="card-hint">${item.tag}</div>` : ''}
  ...
  renderQuiz(q) {
    ...
    const questionLang = direction === 'fi-to-tr' ? 'Fince ifadeyi Türkçeye çevir' : 'Türkçeyi Fincede bul';
    return `
    <div class="quiz-container">
      ...
      ${answered ? `<div class="answer-feedback ${correct?'correct':'wrong'}">${correct?'✅ Doğru!':`❌ Yanlış! Doğrusu: ${direction==='fi-to-tr'?item.tr:item.fi}`}</div>` : ''}
    </div>`;
  }
  ...
  renderMatch(q) {
    ...
    <div class="match-container">
      ...
      <div class="match-columns">
        <div class="match-col">
          ${items.map(item => {
            const isMatched=matched.has(item.id), isSelected=selectedFi===item.id;
            return `<button class="match-btn fi-btn ${isMatched?'matched':''} ${isSelected?'selected':''}" data-action="matchSelect" data-type="fi" data-id="${item.id}" ${isMatched?'disabled':''}>${item.fi}</button>`;
          }).join('')}
        </div>
        <div class="match-col">
          ${items.map(item => {
            const isMatched=matched.has(item.id), isSelected=selectedTr===item.id;
            return `<button class="match-btn tr-btn ${isMatched?'matched':''} ${isSelected?'selected':''}" data-action="matchSelect" data-type="tr" data-id="${item.id}" ${isMatched?'disabled':''}>${item.tr}</button>`;
          }).join('')}
        </div>
      </div>
  ...
  renderListening(q) {
    ...
    <div class="listening-container">
      ...
      ${answered ? `<div class="answer-feedback ${q.correct?'correct':'wrong'}">${q.correct?'✅ Doğru!':`❌ Yanlış! Doğru: ${item.fi}`}</div>` : ''}
    </div>`;
  }

  renderResults() {
    const { sessionCorrect, accuracy, xpEarned } = this.state;
    let stars = '⭐⭐⭐';
    if ((accuracy||0)<70) stars='⭐'; else if ((accuracy||0)<90) stars='⭐⭐';
    return `
    <div class="screen results-screen">
      <div class="results-hero">
        <div class="results-stars">${stars}</div>
        <h1 class="results-title">${(accuracy||0)>=80?'Harika!':(accuracy||0)>=60?'İyi İş!':'Devam Et!'}</h1>
      </div>
      <div class="results-stats">
        <div class="result-stat"><div class="stat-value">${accuracy||0}%</div><div class="stat-label">Doğruluk</div></div>
        <div class="result-stat"><div class="stat-value">${sessionCorrect||0}</div><div class="stat-label">Doğru</div></div>
        <div class="result-stat"><div class="stat-value">+${xpEarned||0}</div><div class="stat-label">XP Kazandı</div></div>
      </div>
  ...
  renderProfile() {
    ...
    <div class="screen profile-screen">
      ...
      <div class="profile-hero">
        ...
        <div class="xp-bar-wrap"><div class="xp-bar" style="width:${Math.min(100,((totalXP||0)%500)/5)}%"></div></div>
        <div class="xp-label">${totalXP||0} XP • ${xpToNext} XP sonraki seviye</div>
      </div>
      <div class="stats-grid">
        <div class="stat-card"><div class="stat-icon">🔥</div><div class="stat-num">${dailyStreak||0}</div><div class="stat-label">Gün Serisi</div></div>
        <div class="stat-card"><div class="stat-icon">📚</div><div class="stat-num">${Object.values(VOCABULARY).flat().length}</div><div class="stat-label">Toplam Kelime</div></div>
        <div class="stat-card"><div class="stat-icon">🏫</div><div class="stat-num">${typeof SCHOOL_SENTENCES!=='undefined'?Object.values(SCHOOL_SENTENCES).flat().length:0}</div><div class="stat-label">Okul Diyaloğu</div></div>
        <div class="stat-card"><div class="stat-icon">🔢</div><div class="stat-num">${typeof MATH_VOCABULARY!=='undefined'?Object.values(MATH_VOCABULARY).flat().length:0}</div><div class="stat-label">Matematik Terimi</div></div>
      </div>
      <div class="level-progress-section">
        ...
        ${['A1','A2','B1','B2'].map(lvl => `
          <div class="level-progress-item">
            <div class="level-badge level-${lvl.toLowerCase()}">${lvl}</div>
            <div class="level-bar-wrap"><div class="level-bar" style="width:${this.progress.levelProgress?.[lvl]||0}%"></div></div>
  ...
  renderWordList() {
    const allWords = Object.entries(VOCABULARY).flatMap(([level,words]) => words.slice(0,20).map(w=>({...w,level})));
    return `
    <div class="screen wordlist-screen">
      ...
      <div class="filter-tabs">
        ${['Tümü','A1','A2','B1','B2'].map(f=>`<button class="filter-tab ${f==='Tümü'?'active':''}" data-filter="${f}">${f}</button>`).join('')}
      </div>
      <div class="word-list">
        ${allWords.map(w=>`
          <div class="word-item">
            <div class="word-fi">${w.fi}<button class="speak-tiny" data-action="speak" data-text="${w.fi}">🔊</button></div>
      ...
      el.addEventListener('click', () => {
        ...
        switch(action) {
          ...
          case 'selectLevel':
            ...
            break;
          case 'startActivity':  this.startActivity(this.state.level, el.dataset.mode, this.state.contentType); break;
          case 'revealCard':
            ...
            break;
          case 'rateCard':      this.handleFlashcard(el.dataset.rating); break;
          case 'quizAnswer':    this.handleQuizAnswer(parseInt(el.dataset.index)); break;
          case 'matchSelect':   this.handleMatchSelect(el.dataset.type, parseInt(el.dataset.id)); break;
          case 'listenAnswer':  this.handleListeningAnswer(parseInt(el.dataset.index)); break;
          case 'speak':         this.speak(el.dataset.text); break;
          case 'retryActivity': this.startActivity(this.state.level, this.state.mode, this.state.contentType); break;
        }
      });
    });
  }
...
document.addEventListener('DOMContentLoaded', () => { window.app = new FinnishApp(); });
if (document.readyState !== 'loading') { if (!window.app) window.app = new FinnishApp(); }
